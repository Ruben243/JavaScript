class UI {
    ...
    mostrarPines(datos) {
        ...
        datos.forEach(dato => {
            ...
            const { geometry: { x, y }, attributes: { Dirección, Precio_gasolina_95, Precio_gasóleo_A, Horario } } = dato;

            //Crear el  pop-up con .popup
            const opcionesPopUp = L.popup()
                .setContent(
                    `<p>Calle:${Dirección}</p>
                    <p>Precio gasolina: ${Precio_gasolina_95}</p>
                    <p>Precio gasóleo A: ${Precio_gasóleo_A}</p>
                    <p>Horario de apertura: ${Horario}</p>`
                );

            //Crea el pin pasandole la longitud y latitud como parametro
            const marker = new L.marker([
                parseFloat(y),//lattitud
                parseFloat(x)///longitud
